...
class Expr:
    ...
    def __hash__(self):
        h = self._mhash
        if h is None:
            h = hash((type(self).__name__,) + tuple(self.args))
            self._mhash = h
        return h
...
class ScalPow(Expr):
    def __new__(cls, *args):
        ...
        if len(instance.args) > 2:
            raise TypeError(
                "ScalPow takes 2 arguments but "
                + str(len(instance.args))
                + " were given."
            )
...
class Mult(Expr, Associative, Commutative, Identity, Cummulative, NullElement):
    def __new__(cls, *args):
        if len(args) <= 1:
        ...
        instance.args = instance.simplify(
            ScalPow, lambda a, b: a + b, instance._separate_exp, instance.args
        )
...
class Plus(Expr, Associative, Commutative, Identity, Cummulative):
    def __new__(cls, *args):
        ...
        if len(instance.args) == 0:
            ...
            return instance.args[0]
        instance.args = instance.simplify(
            Mult, instance._number_version, instance._separate_num, instance.args
        )
        instance.make_commutative()
...
class Deriv(Expr):
    ...
    def latex(self):
        return (
            "\partial_{"
            + " ".join(f.latex() for f in self.coordinate)
            + "}"
            + "("
            + self.base.latex()
            + ")"
...
def dependency(exp, coordinate):
    ...
    if isinstance(exp, Mult) or isinstance(exp, Plus) or isinstance(exp, ScalPow):
...
class Serie(Expr):
    def __init__(self, args=[], coordinate=None, order=None, for_tensor=False):
        if not for_tensor:
            if coordinate == None:
                ...
                if len(coo) > 1:
                    raise ValueError(
                        "For multivalued Series you must "
    ...
    def __repr__(self):
        return " + ".join(repr(self.args[arg]) for arg in self.args) + " + O(ϵ^%s)" % (
            self.order
        )
...
class SeriePow(Serie):
    def __new__(cls, *args):
        ...
        if len(instance.args) > 2:
            raise TypeError(
                "ScalPow takes 2 arguments but " + len(instance.args) + " were given."
            )
...
class PlusSeries(Serie):
    ...
    def simple_distribute(self, args):
        ...
        new_serie = [
            serie_args[i] if i != 0 else serie_args[i] + scalar for i in serie_args
        ]
        return new_serie
